...
P_ray_f = zeros(ray.nElevationRays,ray.nAzimuthRays,nFreq); % pre-allocate P(f) for a beam
P_ray_t = zeros(ray.nElevationRays,ray.nAzimuthRays,nFreq); % pre-allocate P(t) for a beam

% --- Beam loop --- %
% Elevation ray Loop
for k = 1:ray.nElevationRays
    % Azimuth ray Loop
    for i = 1:ray.nAzimuthRays
        ...
        azimuthBeamPattern = (sinc(pi()*0.884/ray.azimuthAngleWidth*sin(ray.azimuthAngles(i)))).^2;
        elevationBeamPattern = (sinc(pi()*0.884/ray.elevationAngleWidth*sin(ray.elevationAngles(k)))).^2;
